...
if( function_exists('vc_add_shortcode_param') ){
	function kwayy_iconselector_func($settings, $value) {
		...
		return '<div class="tm_icon_selector_block">'
...
add_action( 'vc_after_init', 'tm_vc_custom_element_portfoliobox', 21 );
add_action( 'vc_after_init', 'tm_vc_custom_element_team', 21 );
add_action( 'vc_after_init', 'tm_vc_custom_element_testimonial', 21 );
add_action( 'vc_after_init', 'tm_vc_custom_element_clients', 21 );
...
add_action( 'vc_after_init', 'tm_vc_custom_element_icon_separator' );
...
add_action( 'vc_after_init', 'tm_vc_custom_element_eventsbox', 21 );
add_action( 'vc_after_init', 'tm_vc_custom_element_socialbox' );
add_action( 'vc_after_init', 'tm_vc_custom_element_schedulebox' );

/*Service box old element*/
add_action( 'vc_after_init', 'tm_vc_custom_element_servicebox_old' );
...
function tm_vc_custom_element_facts_in_digits(){
	...
	$allParams1 =  array(
		...
		/*array(
			...
			"holder"      => "div",
			"heading"     => __("Icon2", "apicona"),
			"value"       => $allIcons,
			"description" => __("22 Select icon for separator. Only visible if sub-heading is filled.",'apicona'),
			'std'         => 'fa-skype',
		),*/
		...
		array(
			...
			"value"       => thememount_icon_list_array(),
...
function tm_vc_custom_element_servicebox(){
	...
	$boxcolor =   array_merge( $bgcolor_custom , ( ( function_exists('vc_get_shared') ) ? vc_get_shared( 'colors-dashed' ) : getVcShared( 'colors-dashed' ) ) ) ;
...
foreach($kwayy_iconsArray as $icon ){
	...
	$newColorList =   array_merge( $newColors , ( ( function_exists('vc_get_shared') ) ? vc_get_shared( 'colors-dashed' ) : getVcShared( 'colors-dashed' ) ) ) ;
...
vc_map( array(
	...
	"params"   => array(
		...
		array(
			...
			'value' => ( ( function_exists('vc_get_shared') ) ? vc_get_shared( 'button styles' ) : getVcShared( 'button styles' ) ),
		...
		array(
			...
			'value' => ( ( function_exists('vc_get_shared') ) ? vc_get_shared( 'sizes' ) : getVcShared( 'sizes' ) ),
			'std'			=> 'md',
...
function tm_vc_custom_element_team(){
	...
	$allParams = array(
		...
		array(
			...
			"class"       => "",
			"heading"     => __("Order by",'apicona'),
			"description" => sprintf( __("Sort retrieved %s by parameter.", 'apicona'), $team_type_title),
			"param_name"  => "orderby",
			"value"       => array(
				__('No order (none)','apicona')           => 'none',
				__('Order by post id (ID)','apicona')     => 'ID',
...
function tm_vc_custom_element_clients(){
	...
	vc_map( array(
		"name"     => __("ThemeMount Clients Logo Box", "apicona"),
		"base"     => "clients",
		"icon"     => "icon-thememount-vc",
		'category' => __( 'ThemeMount Special Elements', 'apicona' ),
		"params"   => $params,
