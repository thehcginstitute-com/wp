...
?>
	<div<?php echo $inline_css; ?> class="<?php echo esc_attr( trim( $css_class ) ); ?>">
		<?php if ( '' !== $link && 'bottom' !== $position ) {
			echo do_shortcode( '[vc_button2 align="' . $position . '" link="' . $link . '" title="' . $title . '" color="' . $color . '" icon="' . $icon . '" size="' . $size . '" style="' . $btn_style . '" el_class="vc_cta_btn" btnicon="'.$btnicon.'" btniconposition="'.$btniconposition.'"]' );
		} ?>
		<?php if ( '' !== $h2 || '' !== $h4 ) :  ?>
			<hgroup>
