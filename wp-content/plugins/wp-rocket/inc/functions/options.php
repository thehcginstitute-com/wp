 ...
 * 2024-06-26 Dmitrii Fediuk https://upwork.com/fl/mage2pro
 * https://github.com/thehcginstitute-com/wp/blob/2024-02-28-2/wp-content/plugins/wp-rocket/inc/functions/options.php#L451-L470
 * @return bool|array
 */
function rocket_check_key() {
	Logger::info('LICENSE VALIDATION PROCESS STARTED.', ['license validation process']);
	$r = [
		'consumer_email' => 'admin@mage2.pro'
		,'consumer_key' => 'activated'
		,'secret_key' => 'activated'
	];
	if (!get_rocket_option('license')) {
		$r['license'] = '1';
	}
	Logger::info('License validation successful.', ['license validation process']);
	set_transient(rocket_get_constant( 'WP_ROCKET_SLUG' ), $r);
